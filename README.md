...
- The InputPlumber touch-input race fix (one-time and permanent autostart)
- Flatpak app installs (Steam, Chrome, RetroArch, Heroic, Discord, etc.)
- Mode switching (Game Mode and Desktop Mode)
...
After install, restart your agent or IDE so it picks up the new skill.
...
After any of these, restart Claude Code or your agent so it loads the playbook.
...
| File | Purpose |
|---|---|
| [`AGENTS.md`](AGENTS.md) | Canonical agent-agnostic playbook (source of truth) |
| [`SKILL.md`](SKILL.md) | Claude Code wrapper. Frontmatter plus delegation to AGENTS.md |
...
| [`references/troubleshooting.md`](references/troubleshooting.md) | Every issue we've seen, with cause and fix |
...
This playbook wraps the work of **[Luke Short](https://github.com/LukeShortCloud)** ([@LukeShortCloud](https://github.com/LukeShortCloud)), VP of Linux Engineering at Playtron. His [`gameos-unlock`](https://github.com/LukeShortCloud/gameos-unlock) script does the actual heavy lifting: the bootc image build, the desktop-mode installer, the Flatpak wrapper. **None of this would exist without his work.** This repo just teaches AI agents how to walk a user through running it cleanly.
